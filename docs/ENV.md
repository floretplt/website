...
| `ORDER_CHECKOUT_SECRET` | Optional | HMAC secret for the prepay checkout cookie (`floret_order_checkout`). If unset, `LIQPAY_PRIVATE_KEY` is used. Set a dedicated value in production if you rotate LiqPay keys independently of checkout binding. |
